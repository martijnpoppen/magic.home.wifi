...
class Control {
	...
	_sendCommand(buf, expect_reply, callback) {
        ...
        if(self._socket) {
          ...
          self._socket.emit('error', new Error('Connection to device broken'));
        }
			});
	...
	setColor(red, green, blue, callback) {
  	...
  	setWarmWhite(ww, callback) {
		return this.setColorAndWarmWhite(0, 0, 0, ww, callback);
	}
  	...
  	setColorAndWarmWhite(red, green, blue, ww, callback) {
		...
		green = clamp(green, lower_bound, 255);
		blue = clamp(blue, lower_bound, 255);
		ww = clamp(ww, lower_bound, 255);

		var cmd_buf = Buffer.from([0x31, red, green, blue, ww, this._characteristics.set_color_magic_bytes[0], this._characteristics.set_color_magic_bytes[1]]);
